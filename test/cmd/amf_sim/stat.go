...
func NewHTTPMetrics() *HTTPMetrics {
	return &HTTPMetrics{
		...
		reach:         ustat.GetOrRegisterTimer("Reach", nil),
		failnoti:      ustat.GetOrRegisterTimer("FailNoti", nil),
...
func (m *HTTPMetrics) ReachStop(start time.Time, code int) {
	now := time.Now()
	m.reach.Update(now.Sub(start))
	m.Stop(start, code)
}

func (m *HTTPMetrics) FailNotiStop(start time.Time, code int) {
	now := time.Now()
	m.failnoti.Update(now.Sub(start))
	m.Stop(start, code)
}
...
func (m *HTTPMetrics) R_Report(prResult, prTime, prError bool) {
	duration := uint64(1)
	if m.started {
	...
	PrintServiceStat("AMF-Reachiablity", m.reach, duration)
...
// Report HTTP 통계를 출력한다.
func (m *HTTPMetrics) S_Report(prResult, prTime, prError bool) {
	duration := uint64(1)
	if m.started {
		duration = uint64(m.last.Sub(m.first) / time.Second)
		if duration == 0 {
	...
	PrintServiceStat("AMF-Uplink", m.uplink, duration)
	PrintServiceStat("AMF-FailNoti", m.failnoti, duration)
