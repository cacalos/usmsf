...
func (s *NFServer) checkContentType(contentsType string) error {
	...
	if strings.Compare(contentsType, echo.MIMEApplicationJSON) == 0 {
		//"application/json") == 0 {
		err = errors.New("Invalid Parameter : Unsupported Content-Type")
		return nil
	} else if strings.Compare(contentsType, "multipart/related;boundary=Boundary") == 0 {
		return nil
	}
